...
SYN6288语音模块
//!!!!!!!!!!!!!!!!波特率必须9600!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
SYN_FrameInfo("[m1][v1][t5]请在五秒内进行酒精检测"); 
**********************************************************/
...
void SYN_FrameInfo(uint8_t *Voice)   //语音播报函数
{
   ...
   Frame_Info[2]=3+HZ_Length;//关于语音长度
	...
	//不能在近期用串口！！HAL库先发送5个，再发送文字，再改善ecc(校验位)
	 HAL_UART_Transmit(&huart2, (uint8_t *)Frame_Info,5, 0xFFFF);
   HAL_UART_Transmit(&huart2,                 Voice, HZ_Length, 0xFFFF);
	 HAL_UART_Transmit(&huart2, (uint8_t *)&ecc,1, 0xFFFF);
}
